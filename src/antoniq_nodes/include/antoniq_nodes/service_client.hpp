...
#ifndef ANTONIQ_NODES__SERVICE_CLIENT_HPP_
#define ANTONIQ_NODES__SERVICE_CLIENT_HPP_

#include <string>

#include "rclcpp/rclcpp.hpp"

namespace robs4crop_util
{

/**
 * @class robs4crop_util::ServiceClient
...
public:
  ...
  invoke(typename RequestType::SharedPtr &request,

         const std::chrono::nanoseconds timeout = std::chrono::nanoseconds(-1),
         const std::chrono::seconds wait_timeout = std::chrono::seconds(5), const int retries = 1)
  {
    int retry = 4;
    for (size_t i = 0; i < retry; i++) {
      ...
      if (callback_group_executor_.spin_until_future_complete(future_result, 5s) !=
          rclcpp::FutureReturnCode::SUCCESS) {
        // Pending request must be manually cleaned up if execution is
        // interrupted or timed out
        client_->remove_pending_request(future_result);
        RCLCPP_ERROR(node_->get_logger(), "%s service client: async_send_request failed",
                     service_name_.c_str());
        if (i == retry - 1) {
...
#endif  // ANTONIQ_NODES__SERVICE_CLIENT_HPP_
